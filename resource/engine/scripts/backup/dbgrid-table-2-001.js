	...
	if(column.command) {
		...
		if(column.internal) {
			if(column.command == "select") {
				...
				command = desktop.svg.draw(cell, self.getCommandHeaderIcon(column))
			} else {
				desktop.svg.draw(cell, "db-" + column.command)
			}
		} else {
			// var hint = column.grid.methods.call("getCommandHint", column)
			var hint = self.getCommandHint(column);
			if(hint !=== "") {
				desktop.SetHint(cell, hint, "top")
			}
...
jTableContentPainter.prototype.drawCell = function(cell, column) {
	...
	if(column.command) {
		...
		if(column.internal) {
			...
			} else if(column.grid.methods.call("allowCommand", column)) {
